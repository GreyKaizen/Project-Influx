...
class Invoice
{
    ...
    /* Setter Function For The Invoice Type */
        ...
        friend void print_Invoice(const Buyer&,const Order&,const Payment&,const vector <Cart>&, Invoice&);//DONE but NEED UI
...
class Order{
    ...
    public:
        ...
        Payment getPayment();
        string getOrderId() const;  //done
        // int getQuantity() const;    //done
        //No need to return a const Amount since we'll be using it to perform other operations
        double getAmount();      //done
